...
pixel_results <- data.frame(Type = NA, Planted = NA, 
                            ...
                            ETH = 1, CHM = 1, PS =1, SWF = 1, TOW = 1,
                            DF =1 , DF_trained =1, COuntry = NA)
...
#rewriting all of the rasters
  ...
  ETH_tmp <- resample(ETH, CHM,
                      method = "near")
  PS_tmp <- project(PS, CHM, method = "near")
  CHM2 <- CHM
  values(CHM2) <- 0
  ...
  tmp <- data.frame(Type = studysites$Type[i], Planted = studysites$Planted[i], 
                    ...
                    CHM = extract(CHM, studysites[i,],
                                  ID = FALSE),
                    PS = extract(PS_tmp, studysites[i,],
                                  ID = FALSE), 
                    SWF = extract(SWF, studysites[i,],
                                  ID = FALSE),
                    TOW = extract(TOW, studysites[i,],
                                  ID = FALSE),
                    DF = extract(DF, studysites[i,],
                                 ID = FALSE),
                    DF_trained = extract(DF2, studysites[i,],
                                         ID = FALSE),
                    Country = studysites$Country[i])
  #remaming
...
#changing NAs in SWF, TOW, and DF to 0
pixel_results2$SWF[is.na(pixel_results2$SWF)] <- 0
pixel_results2$TOW[is.na(pixel_results2$TOW)] <- 0
pixel_results2$DF[is.na(pixel_results2$DF)] <- 0
pixel_results2$DF_trained[is.na(pixel_results2$DF_trained)] <- 0
...
#parcel based
  ...
  for(i in 1:length(studysites)) {
    ...
    parcel_results$CHM[i] <- (sum(as.numeric(extract(CHM, area, ID = FALSE)>=3))/length(unlist(
...
##weighted kappa calculation, uses same code as omissions and comission to start
...
#parcel calculation, now using UKCEH or OSMM maps
for(i in 1:length(studysites)) {
  ...
  for(j in 1:length(parcels)) {
    ...
    area2 <- buffer(parcels[j,], width = -10) # for the treecover analsis
    #other stuff
    ...
    if(length(expanse(area)) != 0) {
      ...
      DF <- vect("C:/Users/am1355/OneDrive - University of Leicester/misc/trees/trees_extra.gpkg",
                 extent = ext(area2))
      DF <- erase(DF) #need to do this due to overlaps
      DF2 <- vect("C:/Users/am1355/OneDrive - University of Leicester/misc/trees/DF_trees_trained2.gpkg",
                  extent = ext(area2))
      DF2 <- erase(DF2)
      #bluesky
      NTM <- vect("C:/Users/am1355/OneDrive - University of Leicester/misc/trees/bluesky_NTM_full.gpkg",
                  extent = ext(area2))
      #meta
      ...
      temp$ETH <- (sum(as.numeric(extract(ETH, area2, ID = FALSE)>=10))/length(unlist(
        extract(ETH, area, ID = FALSE))))*100
      ...
      #PS-CHM
      temp$PS <- (sum(as.numeric(extract(PS, area2, ID = FALSE)>=10))/length(unlist(
        extract(meta_trees, area2, ID = FALSE))))*100
      #temp$CHM <- unlist(zonal(meta_trees, area2, weights = TRUE))
      temp$TOW <- (sum(expanse(terra::intersect(trees, area2)))/expanse(area2))*100
      temp$NTM <- (sum(expanse(terra::intersect(NTM, area2)))/expanse(area2))*100
      ...
      temp$DF_trained <- (sum(expanse(terra::intersect(DF2, area2)))/expanse(area2))*100
...
parcel_results2$UKCEH <- factor(parcel_results2$UKCEH, 
                                ...
                                labels = c("Forest", "Forest", "Arable",
                                           ...
                                           "Wetland", "Shrub", "Grass", "Wetland",
                                           "Other", "Other"))
#getting a second type
parcel_results2$Type_simp <- NA
parcel_results2$Type_simp[parcel_results2$Type %in% c("SA", "SP")] <- "AF"
parcel_results2$Type_simp[!(parcel_results2$Type %in% c("SA", "SP"))] <- "C"
#setting out numbers
#treecovs n order TCD10m 100m, ETH, CHM, PS, SWFm TOW, NTM,DF, DF-trained
treecov_nums <- rep(c(5,6,7,11,8,10,9,12,13,14),4)
#ladncovs in order of UKCEH, CORINE< ODSE, OSMLU
landcov_nums <- c(rep(4,10), rep(1,10),
                  rep(2,10), rep(3,10))

kappas_array <- data.frame(landcov = rep(NA, length(treecov_nums)), 
                           treecov = NA, kappa = NA)
kappas_array$landcov <- c(rep("UKCEH",10), rep("CORINE",10),
                          rep("ODSE-LU",10), rep("OSM-LU",10))
kappas_array$treecov <- rep(c("TCD 10m", "TCD 100m",
                              "ETH-CHM", "Meta-CHM",
                              "PS-CHM", "SWF", "TOW",
                              "NTM", "DF", "DF-trained"),4)
#setting a custom function for a weighted kappa
weighted_kappa <- function(rater1, rater2, weights) {
  stopifnot(length(rater1) == length(rater2), length(weights) == length(rater1))
  ...
  # observed agreement
  P_o <- sum(w * (rater1 == rater2))
  
  # expected agreement
  p1 <- sapply(cats, function(k) sum(w * (rater1 == k)))
  p2 <- sapply(cats, function(k) sum(w * (rater2 == k)))
  P_e <- sum(p1 * p2)
...
#trying a balanced accuracy metric instead here
weighted_balanced_accuracy <- function(true, pred, weights) {
  stopifnot(length(true) == length(pred), length(weights) == length(true))
  classes <- union(unique(true), unique(pred))
  
  recalls <- sapply(classes, function(k) {
    # Weighted true positives
    tp <- sum(weights[true == k & pred == k])
    # Weighted total actuals for class k
    total_actual <- sum(weights[true == k])
    if (total_actual == 0) return(NA)
    tp / total_actual
  })
...
#weighred MCC now
weighted_mcc <- function(true, pred, weights) {
  stopifnot(length(true) == length(pred), length(pred) == length(weights))
  
  # Convert to factors with consistent levels
  true <- factor(true)
  pred <- factor(pred, levels = levels(true))
  if (length(levels(true)) != 2) stop("Binary labels required.")
  
  # Extract class labels
  pos <- levels(true)[1]
  neg <- levels(true)[2]
  
  TP <- sum(weights[true == pos & pred == pos])
  TN <- sum(weights[true == neg & pred == neg])
  FP <- sum(weights[true == neg & pred == pos])
  FN <- sum(weights[true == pos & pred == neg])
...
for(i in 1:nrow(kappas_array)) {
  #note i need to do something different for the TOW and NTM
  if(kappas_array$treecov[i] == "NTM") {
    parcel_results3 <- parcel_results2[which(parcel_results2$Country != "Northern Ireland"),]
    preds <- rep("C", nrow(parcel_results3))
    preds[which(parcel_results3[,treecov_nums[i]] >= 5 &
                  parcel_results3[,landcov_nums[i]] %in% c("Pasture", "Arable"))] <- "AF"
    kappas_array$kappa[i] <- weighted_kappa(parcel_results3$Type_simp, preds,
                                            parcel_results3$overlap_a)
  } else {
    if(kappas_array$treecov[i] == "TOW") {
      parcel_results3 <- parcel_results2[which(parcel_results2$Country == "England"),]
      preds <- rep("C", nrow(parcel_results3))
      preds[which(parcel_results3[,treecov_nums[i]] >= 5 &
                    parcel_results3[,landcov_nums[i]] %in% c("Pasture", "Arable"))] <- "AF"
      kappas_array$kappa[i] <- weighted_kappa(parcel_results3$Type_simp, preds,
      ...
      #getting list of predicted
      preds <- rep("C", nrow(parcel_results2))
      #setting agroforesty sites
      preds[which(parcel_results2[,treecov_nums[i]] >= 5 &
                    parcel_results2[,landcov_nums[i]] %in% c("Pasture", "Arable"))] <- "AF"
      kappas_array$kappa[i] <- weighted_kappa(parcel_results2$Type_simp, preds,
                                              parcel_results2$overlap_a)
      #note i need to do something different for the
    }
  }
...
#for mature kappas
kappas_mat_array <- data.frame(landcov = rep(NA, length(treecov_nums)), 
                           treecov = NA, kappa = NA)
kappas_mat_array$landcov <- c(rep("UKCEH",10), rep("CORINE",10),
                          rep("ODSE-LU",10), rep("OSM-LU",10))
kappas_mat_array$treecov <- rep(c("TCD 100m", "TCD 10m",
                              "ETH-CHM", "Meta-CHM",
                              "PS-CHM", "SWF", "TOW",
                              "NTM", "DF", "DF-trained"),4)
parcel_results3 <- parcel_results2[which(parcel_results2$Planted <= 2000 |
                                           is.na(parcel_results2$Planted)),]
#note we are only removing the agroforestry fields, not the controls in these areas
for(i in 1:nrow(kappas_mat_array)) {
  #note i need to do something different for the TOW and NTM
  if(kappas_mat_array$treecov[i] == "NTM") {
    parcel_results4 <- parcel_results3[which(parcel_results3$Country != "Northern Ireland"),]
    preds <- rep("C", nrow(parcel_results4))
    preds[which(parcel_results4[,treecov_nums[i]] >= 5 &
                  parcel_results4[,landcov_nums[i]] %in% c("Pasture", "Arable"))] <- "AF"
    kappas_mat_array$kappa[i] <- weighted_kappa(parcel_results4$Type_simp, preds,
                                            parcel_results4$overlap_a)
  } else {
    if(kappas_mat_array$treecov[i] == "TOW") {
      parcel_results4 <- parcel_results3[which(parcel_results3$Country == "England"),]
      preds <- rep("C", nrow(parcel_results4))
      preds[which(parcel_results4[,treecov_nums[i]] >= 5 &
                    parcel_results4[,landcov_nums[i]] %in% c("Pasture", "Arable"))] <- "AF"
      kappas_mat_array$kappa[i] <- weighted_kappa(parcel_results4$Type_simp, preds,
      ...
      preds[which(parcel_results3[,treecov_nums[i]] >= 5 &
                    parcel_results3[,landcov_nums[i]] %in% c("Pasture", "Arable"))] <- "AF"
      kappas_mat_array$kappa[i] <- weighted_kappa(parcel_results3$Type_simp, preds,
                                              parcel_results3$overlap_a)
      #note i need to do something different for the
    }
  }
...
#getting array
accuracy_array <- data.frame(landcov = rep(NA, length(treecov_nums)), 
                           treecov = NA, accuracy = NA)
accuracy_array$landcov <- c(rep("UKCEH",10), rep("CORINE",10),
                          rep("ODSE-LU",10), rep("OSM-LU",10))
accuracy_array$treecov <- rep(c("TCD 10m", "TCD 100m",
                              "ETH-CHM", "Meta-CHM",
                              "PS-CHM", "SWF", "TOW",
                              "NTM", "DF", "DF-trained"),4)
#for mature accuracy
accuracy_mat_array <- data.frame(landcov = rep(NA, length(treecov_nums)), 
                               treecov = NA, kappa = NA)
accuracy_mat_array$landcov <- c(rep("UKCEH",10), rep("CORINE",10),
                              rep("ODSE-LU",10), rep("OSM-LU",10))
accuracy_mat_array$treecov <- rep(c("TCD 10m", "TCD 100m",
                                  "ETH-CHM", "Meta-CHM",
                                  "PS-CHM", "SWF", "TOW",
                                  "NTM", "DF", "DF-trained"),4)
parcel_results3 <- parcel_results2[which(parcel_results2$Planted <= 2000 |
                                           is.na(parcel_results2$Planted)),]
#now calculating
for(i in 1:nrow(accuracy_array)) {
  #note i need to do something different for the TOW and NTM
  if(accuracy_array$treecov[i] == "NTM") {
    parcel_results3 <- parcel_results2[which(parcel_results2$Country != "Northern Ireland"),]
    preds <- rep("C", nrow(parcel_results3))
    preds[which(parcel_results3[,treecov_nums[i]] >= 5 &
                  parcel_results3[,landcov_nums[i]] %in% c("Pasture", "Arable"))] <- "AF"
    accuracy_array$accuracy[i] <- weighted_balanced_accuracy(parcel_results3$Type_simp, preds,
                                            parcel_results3$overlap_a)
  } else {
    if(accuracy_array$treecov[i] == "TOW") {
      parcel_results3 <- parcel_results2[which(parcel_results2$Country == "England"),]
      preds <- rep("C", nrow(parcel_results3))
      preds[which(parcel_results3[,treecov_nums[i]] >= 5 &
                    parcel_results3[,landcov_nums[i]] %in% c("Pasture", "Arable"))] <- "AF"
      ...
      preds[which(parcel_results2[,treecov_nums[i]] >= 5 &
                    parcel_results2[,landcov_nums[i]] %in% c("Pasture", "Arable"))] <- "AF"
      accuracy_array$accuracy[i] <- weighted_balanced_accuracy(parcel_results2$Type_simp, preds,
                                              parcel_results2$overlap_a)
      #note i need to do something different for the
    }
  }
...
#for the mature sites only
parcel_results3 <- parcel_results2[which(parcel_results2$Planted <= 2000 |
                                           is.na(parcel_results2$Planted)),]
for(i in 1:nrow(accuracy_mat_array)) {
  #note i need to do something different for the TOW and NTM
  if(accuracy_mat_array$treecov[i] == "NTM") {
    parcel_results4 <- parcel_results3[which(parcel_results3$Country != "Northern Ireland"),]
    preds <- rep("C", nrow(parcel_results4))
    preds[which(parcel_results4[,treecov_nums[i]] >= 5 &
                  parcel_results4[,landcov_nums[i]] %in% c("Pasture", "Arable"))] <- "AF"
    accuracy_mat_array$accuracy[i] <- weighted_balanced_accuracy(parcel_results4$Type_simp, preds,
                                                parcel_results4$overlap_a)
  } else {
    if(accuracy_mat_array$treecov[i] == "TOW") {
      parcel_results4 <- parcel_results3[which(parcel_results3$Country == "England"),]
      preds <- rep("C", nrow(parcel_results4))
      preds[which(parcel_results4[,treecov_nums[i]] >= 5 &
                    parcel_results4[,landcov_nums[i]] %in% c("Pasture", "Arable"))] <- "AF"
      ...
      preds[which(parcel_results3[,treecov_nums[i]] >= 5 &
                    parcel_results3[,landcov_nums[i]] %in% c("Pasture", "Arable"))] <- "AF"
      accuracy_mat_array$accuracy[i] <- weighted_balanced_accuracy(parcel_results3$Type_simp, preds,
                                                  parcel_results3$overlap_a)
      #note i need to do something different for the
    }
  }
...
#MCC now
mcc_array <- data.frame(landcov = rep(NA, length(treecov_nums)), 
                             treecov = NA, mcc = NA)
mcc_array$landcov <- c(rep("UKCEH",10), rep("CORINE",10),
                            rep("ODSE-LU",10), rep("OSM-LU",10))
mcc_array$treecov <- rep(c("TCD 10m", "TCD 100m",
                                "ETH-CHM", "Meta-CHM",
                                "PS-CHM", "SWF", "TOW",
                                "NTM", "DF", "DF-trained"),4)
#for mature mcc
mcc_mat_array <- data.frame(landcov = rep(NA, length(treecov_nums)), 
                                 treecov = NA, mcc = NA)
mcc_mat_array$landcov <- c(rep("UKCEH",10), rep("CORINE",10),
                                rep("ODSE-LU",10), rep("OSM-LU",10))
mcc_mat_array$treecov <- rep(c("TCD 10m", "TCD 100m",
                                    "ETH-CHM", "Meta-CHM",
                                    "PS-CHM", "SWF", "TOW",
                                    "NTM", "DF", "DF-trained"),4)

#now calculating
for(i in 1:nrow(mcc_array)) {
  #note i need to do something different for the TOW and NTM
  if(mcc_array$treecov[i] == "NTM") {
    parcel_results3 <- parcel_results2[which(parcel_results2$Country != "Northern Ireland"),]
    preds <- rep("C", nrow(parcel_results3))
    preds[which(parcel_results3[,treecov_nums[i]] >= 5 &
                  parcel_results3[,landcov_nums[i]] %in% c("Pasture", "Arable"))] <- "AF"
    mcc_array$mcc[i] <- weighted_mcc(parcel_results3$Type_simp, preds,
                                                             parcel_results3$overlap_a)
  } else {
    if(mcc_array$treecov[i] == "TOW") {
      parcel_results3 <- parcel_results2[which(parcel_results2$Country == "England"),]
      preds <- rep("C", nrow(parcel_results3))
      preds[which(parcel_results3[,treecov_nums[i]] >= 5 &
                    parcel_results3[,landcov_nums[i]] %in% c("Pasture", "Arable"))] <- "AF"
      ...
      preds[which(parcel_results2[,treecov_nums[i]] >= 5 &
                    parcel_results2[,landcov_nums[i]] %in% c("Pasture", "Arable"))] <- "AF"
      mcc_array$mcc[i] <- weighted_mcc(parcel_results2$Type_simp, preds,
                                                               parcel_results2$overlap_a)
      #note i need to do something different for the
    }
  }
...
parcel_results3 <- parcel_results2[which(parcel_results2$Planted <= 2000 |
                                           is.na(parcel_results2$Planted)),]
for(i in 1:nrow(mcc_mat_array)) {
  #note i need to do something different for the TOW and NTM
  if(mcc_mat_array$treecov[i] == "NTM") {
    parcel_results4 <- parcel_results3[which(parcel_results3$Country != "Northern Ireland"),]
    preds <- rep("C", nrow(parcel_results4))
    preds[which(parcel_results4[,treecov_nums[i]] >= 5 &
                  parcel_results4[,landcov_nums[i]] %in% c("Pasture", "Arable"))] <- "AF"
    mcc_mat_array$mcc[i] <- weighted_mcc(parcel_results4$Type_simp, preds,
                                                                 parcel_results4$overlap_a)
  } else {
    if(mcc_mat_array$treecov[i] == "TOW") {
      parcel_results4 <- parcel_results3[which(parcel_results3$Country == "England"),]
      preds <- rep("C", nrow(parcel_results4))
      preds[which(parcel_results4[,treecov_nums[i]] >= 5 &
                    parcel_results4[,landcov_nums[i]] %in% c("Pasture", "Arable"))] <- "AF"
      ...
      preds[which(parcel_results3[,treecov_nums[i]] >= 5 &
                    parcel_results3[,landcov_nums[i]] %in% c("Pasture", "Arable"))] <- "AF"
      mcc_mat_array$mcc[i] <- weighted_mcc(parcel_results3$Type_simp, preds,
                                                                   parcel_results3$overlap_a)
      #note i need to do something different for the
    }
  }
...
#pixel based balanced accuracies
thresholds <- rep(c(5,5,10,3,10,1,1,1,1), 4)
#treecovs n order TCD10m 100m, ETH, CHM, PS, SWFm TOW, NTM,DF, DF-trained
treecov_nums <- rep(c(8,9,10,11,12,13,14,15,16),4)
#ladncovs in order of UKCEH, CORINE< ODSE, OSMLU
landcov_nums <- c(rep(7,9), rep(4,9),
                  rep(5,9), rep(6,9))
pixel_accuracy_array <- data.frame(landcov = rep(NA, length(treecov_nums)), 
                               treecov = NA, accuracy = NA)
pixel_accuracy_array$landcov <- c(rep("UKCEH",9), rep("CORINE",9),
                            rep("ODSE-LU",9), rep("OSM-LU",9))
pixel_accuracy_array$treecov <- rep(c("TCD 10m", "TCD 100m",
                                "ETH-CHM", "Meta-CHM",
                                "PS-CHM", "SWF", "TOW",
                                "DF", "DF-trained"),4)

#simplifying 
pixel_results2$Type2 <- "C"
pixel_results2$Type2[which(pixel_results2$Type == "SA" |
                             pixel_results2$Type == "SP" )] <- "AF"
library(caret)
for(i in 1:nrow(pixel_accuracy_array)) {
  #note i need to do something different for the TOW and NTM
  if(pixel_accuracy_array$treecov[i] == "NTM") {

  } else {
    if(pixel_accuracy_array$treecov[i] == "TOW") {
      pixel_results3 <- pixel_results2[which(pixel_results2$COuntry == "England"),]
      preds <- rep("C", nrow(pixel_results3))
      preds[which(pixel_results3[,treecov_nums[i]] >= thresholds[i] &
                    pixel_results3[,landcov_nums[i]] %in% c("Pasture", "Arable"))] <- "AF"
      pixel_accuracy_array$accuracy[i] <- (sensitivity(reference = factor(pixel_results3$Type2),
                                                              data = factor(preds)) +
                                             specificity(reference = factor(pixel_results3$Type2),
                                                                data = factor(preds)))/2
    } else {
      #getting list of predicted
      preds <- rep("C", nrow(pixel_results2))
      #setting agroforesty sites
      preds[which(pixel_results2[,treecov_nums[i]] >= thresholds[i] &
                    pixel_results2[,landcov_nums[i]] %in% c("Pasture", "Arable"))] <- "AF"
      pixel_accuracy_array$accuracy[i] <- (sensitivity(reference = factor(pixel_results2$Type2),
                                                    data = factor(preds)) +
                                             specificity(reference = factor(pixel_results2$Type2),
                                                                data = factor(preds)))/2
      #note i need to do something different for the
    }
  }
...
#now for mature accuracy
pixel_results3 <- pixel_results2[which(pixel_results2$Planted <= 2000 |
                                           is.na(pixel_results2$Planted)),]

pixel_mat_accuracy_array <- data.frame(landcov = rep(NA, length(treecov_nums)), 
                                   treecov = NA, accuracy = NA)
pixel_mat_accuracy_array$landcov <- c(rep("UKCEH",9), rep("CORINE",9),
                                  rep("ODSE-LU",9), rep("OSM-LU",9))
pixel_mat_accuracy_array$treecov <- rep(c("TCD 10m", "TCD 100m",
                                      "ETH-CHM", "Meta-CHM",
                                      "PS-CHM", "SWF", "TOW",
                                      "DF", "DF-trained"),4)

for(i in 1:nrow(pixel_mat_accuracy_array)) {
  #note i need to do something different for the TOW and NTM
  if(pixel_mat_accuracy_array$treecov[i] == "NTM") {
    
  } else {
    if(pixel_mat_accuracy_array$treecov[i] == "TOW") {
      pixel_results4 <- pixel_results3[which(pixel_results3$COuntry == "England"),]
      preds <- rep("C", nrow(pixel_results4))
      preds[which(pixel_results4[,treecov_nums[i]] >= thresholds[i] &
                    pixel_results4[,landcov_nums[i]] %in% c("Pasture", "Arable"))] <- "AF"
      pixel_mat_accuracy_array$accuracy[i] <- (sensitivity(reference = factor(pixel_results4$Type2),
                                                       data = factor(preds)) +
                                             specificity(reference = factor(pixel_results4$Type2),
                                                         data = factor(preds)))/2
    } else {
      #getting list of predicted
      preds <- rep("C", nrow(pixel_results3))
      ...
      preds[which(pixel_results3[,treecov_nums[i]] >= thresholds[i] &
                    pixel_results3[,landcov_nums[i]] %in% c("Pasture", "Arable"))] <- "AF"
      pixel_mat_accuracy_array$accuracy[i] <- (sensitivity(reference = factor(pixel_results3$Type2),
                                                       data = factor(preds)) +
                                             specificity(reference = factor(pixel_results3$Type2),
                                                         data = factor(preds)))/2
      #note i need to do something different for the
...
#for the kappa now
pixel_kappa_array <- data.frame(landcov = rep(NA, length(treecov_nums)), 
                                   treecov = NA, kappa = NA)
pixel_kappa_array$landcov <- c(rep("UKCEH",9), rep("CORINE",9),
                                  rep("ODSE-LU",9), rep("OSM-LU",9))
pixel_kappa_array$treecov <- rep(c("TCD 10m", "TCD 100m",
                                      "ETH-CHM", "Meta-CHM",
                                      "PS-CHM", "SWF", "TOW",
                                      "DF", "DF-trained"),4)

for(i in 1:nrow(pixel_kappa_array)) {
  #note i need to do something different for the TOW and NTM
  if(pixel_kappa_array$treecov[i] == "NTM") {
    
  } else {
    if(pixel_kappa_array$treecov[i] == "TOW") {
      pixel_results3 <- pixel_results2[which(pixel_results2$COuntry == "England"),]
      preds <- rep("C", nrow(pixel_results3))
      preds[which(pixel_results3[,treecov_nums[i]] >= thresholds[i] &
                    pixel_results3[,landcov_nums[i]] %in% c("Pasture", "Arable"))] <- "AF"
      pixel_kappa_array$kappa[i] <- weighted_kappa(pixel_results3$Type2, preds, rep(1, length(preds)))
    } else {
      #getting list of predicted
      preds <- rep("C", nrow(pixel_results2))
      #setting agroforesty sites
      preds[which(pixel_results2[,treecov_nums[i]] >= thresholds[i] &
                    pixel_results2[,landcov_nums[i]] %in% c("Pasture", "Arable"))] <- "AF"
      pixel_kappa_array$kappa[i] <- weighted_kappa(pixel_results2$Type2, preds, rep(1, length(preds)))
      #note i need to do something different for the
    }
  }
  
}
#now for mature kappa
pixel_results3 <- pixel_results2[which(pixel_results2$Planted <= 2000 |
                                         is.na(pixel_results2$Planted)),]

pixel_mat_kappa_array <- data.frame(landcov = rep(NA, length(treecov_nums)), 
                                       treecov = NA, accuracy = NA)
pixel_mat_kappa_array$landcov <- c(rep("UKCEH",9), rep("CORINE",9),
                                      rep("ODSE-LU",9), rep("OSM-LU",9))
pixel_mat_kappa_array$treecov <- rep(c("TCD 10m", "TCD 100m",
                                          "ETH-CHM", "Meta-CHM",
                                          "PS-CHM", "SWF", "TOW",
                                          "DF", "DF-trained"),4)

for(i in 1:nrow(pixel_mat_kappa_array)) {
  #note i need to do something different for the TOW and NTM
  if(pixel_mat_kappa_array$treecov[i] == "NTM") {
    
  } else {
    if(pixel_mat_kappa_array$treecov[i] == "TOW") {
      pixel_results4 <- pixel_results3[which(pixel_results3$COuntry == "England"),]
      preds <- rep("C", nrow(pixel_results4))
      preds[which(pixel_results4[,treecov_nums[i]] >= thresholds[i] &
                    pixel_results4[,landcov_nums[i]] %in% c("Pasture", "Arable"))] <- "AF"
      pixel_mat_kappa_array$accuracy[i] <- weighted_kappa(pixel_results4$Type2, preds, rep(1, length(preds)))
    } else {
      #getting list of predicted
      preds <- rep("C", nrow(pixel_results3))
      #setting agroforesty sites
      preds[which(pixel_results3[,treecov_nums[i]] >= thresholds[i] &
                    pixel_results3[,landcov_nums[i]] %in% c("Pasture", "Arable"))] <- "AF"
      pixel_mat_kappa_array$accuracy[i] <- weighted_kappa(pixel_results3$Type2, preds, rep(1, length(preds)))
      #note i need to do something different for the
    }
  }
...
#pixel MCC
pixel_mcc_array <- data.frame(landcov = rep(NA, length(treecov_nums)), 
                                treecov = NA, mcc = NA)
pixel_mcc_array$landcov <- c(rep("UKCEH",9), rep("CORINE",9),
                               rep("ODSE-LU",9), rep("OSM-LU",9))
pixel_mcc_array$treecov <- rep(c("TCD 10m", "TCD 100m",
                                   "ETH-CHM", "Meta-CHM",
                                   "PS-CHM", "SWF", "TOW",
                                   "DF", "DF-trained"),4)

for(i in 1:nrow(pixel_mcc_array)) {
  #note i need to do something different for the TOW and NTM
  if(pixel_mcc_array$treecov[i] == "NTM") {
    
  } else {
    if(pixel_mcc_array$treecov[i] == "TOW") {
      pixel_results3 <- pixel_results2[which(pixel_results2$COuntry == "England"),]
      preds <- rep("C", nrow(pixel_results3))
      preds[which(pixel_results3[,treecov_nums[i]] >= thresholds[i] &
                    pixel_results3[,landcov_nums[i]] %in% c("Pasture", "Arable"))] <- "AF"
      pixel_mcc_array$mcc[i] <- weighted_mcc(pixel_results3$Type2, preds, rep(1, length(preds)))
    } else {
      #getting list of predicted
      preds <- rep("C", nrow(pixel_results2))
      #setting agroforesty sites
      preds[which(pixel_results2[,treecov_nums[i]] >= thresholds[i] &
                    pixel_results2[,landcov_nums[i]] %in% c("Pasture", "Arable"))] <- "AF"
      pixel_mcc_array$mcc[i] <- weighted_mcc(pixel_results2$Type2, preds, rep(1, length(preds)))
      #note i need to do something different for the
    }
  }
...
pixel_mat_mcc_array <- data.frame(landcov = rep(NA, length(treecov_nums)), 
                                    treecov = NA, accuracy = NA)
pixel_mat_mcc_array$landcov <- c(rep("UKCEH",9), rep("CORINE",9),
                                   rep("ODSE-LU",9), rep("OSM-LU",9))
pixel_mat_mcc_array$treecov <- rep(c("TCD 10m", "TCD 100m",
                                       "ETH-CHM", "Meta-CHM",
                                       "PS-CHM", "SWF", "TOW",
                                       "DF", "DF-trained"),4)

for(i in 1:nrow(pixel_mat_mcc_array)) {
  #note i need to do something different for the TOW and NTM
  if(pixel_mat_mcc_array$treecov[i] == "NTM") {
    
  } else {
    if(pixel_mat_mcc_array$treecov[i] == "TOW") {
      pixel_results4 <- pixel_results3[which(pixel_results3$COuntry == "England"),]
      preds <- rep("C", nrow(pixel_results4))
      preds[which(pixel_results4[,treecov_nums[i]] >= thresholds[i] &
                    pixel_results4[,landcov_nums[i]] %in% c("Pasture", "Arable"))] <- "AF"
      pixel_mat_mcc_array$accuracy[i] <- weighted_mcc(pixel_results4$Type2, preds, rep(1, length(preds)))
    } else {
      #getting list of predicted
      preds <- rep("C", nrow(pixel_results3))
      #setting agroforesty sites
      preds[which(pixel_results3[,treecov_nums[i]] >= thresholds[i] &
                    pixel_results3[,landcov_nums[i]] %in% c("Pasture", "Arable"))] <- "AF"
      pixel_mat_mcc_array$accuracy[i] <- weighted_mcc(pixel_results3$Type2, preds, rep(1, length(preds)))
      #note i need to do something different for the
    }
  }
...
#plotting these out now
#removing bluesky

#accuracy_array <- accuracy_array[-(which(accuracy_array$treecov == "NTM")),]
#accuracy_mat_array <- accuracy_mat_array[-(which(accuracy_mat_array$treecov == "NTM"))]
# Heatmap
#accuracy
parcel_all <- ggplot( accuracy_array[-(which(accuracy_array$treecov == "NTM")),], 
                      aes(x = landcov, y = treecov, fill = accuracy)) +
  geom_tile(color = "white", linewidth = 0.5) +
  geom_text(aes(label = sprintf("%.2f", accuracy)), color = "black", size = 3) +
  ...
  ggtitle("Parcel-based")
#kappa
parcel_all2 <- ggplot( kappas_array[-(which(kappas_array$treecov == "NTM")),], 
                      aes(x = landcov, y = treecov, fill = kappa)) +
  geom_tile(color = "white", linewidth = 0.5) +
  ...
  ggtitle("Parcel-based")
#mcc
parcel_all3 <- ggplot( mcc_array[-(which(mcc_array$treecov == "NTM")),], 
                       aes(x = landcov, y = treecov, fill = mcc)) +
  geom_tile(color = "white", linewidth = 0.5) +
...
#for the mature sites
#accuracy
parcel_mat <- ggplot(accuracy_mat_array[-(which(accuracy_mat_array$treecov == "NTM")),], 
                     aes(x = landcov, y = treecov, fill = accuracy)) +
  geom_tile(color = "white", linewidth = 0.5) +
  geom_text(aes(label = sprintf("%.2f", accuracy)), color = "black", size = 3) +
  scale_fill_viridis_c(limits = c(0,1), option = "plasma") +
  ...
  ggtitle("Mature Parcel-based")
#kappa
parcel_mat2 <- ggplot(kappas_mat_array[-(which(kappas_mat_array$treecov == "NTM")),], 
                     aes(x = landcov, y = treecov, fill = kappa)) +
  geom_tile(color = "white", linewidth = 0.5) +
  geom_text(aes(label = sprintf("%.2f", kappa)), color = "black", size = 3) +
  scale_fill_viridis_c(limits = c(-1,1), option = "plasma") +
  ...
  ggtitle("Mature Parcel-based")
#MCC
parcel_mat3 <- ggplot(mcc_mat_array[-(which(mcc_mat_array$treecov == "NTM")),], 
                      aes(x = landcov, y = treecov, fill = mcc)) +
  geom_tile(color = "white", linewidth = 0.5) +
  geom_text(aes(label = sprintf("%.2f", mcc)), color = "black", size = 3) +
  scale_fill_viridis_c(limits = c(-1,1), option = "plasma") +
...
#pixel
pixel_all <-  ggplot(pixel_accuracy_array, aes(x = landcov, y = treecov, fill = accuracy)) +
  geom_tile(color = "white", linewidth = 0.5) +
  geom_text(aes(label = sprintf("%.2f", accuracy)), color = "black", size = 3) +
  scale_fill_viridis_c(limits = c(0,1), option = "plasma") +
  theme_minimal(base_size = 12) +
  ...
  ggtitle("Pixel-based")
#kappa
pixel_all2 <-  ggplot(pixel_kappa_array, aes(x = landcov, y = treecov, fill = kappa)) +
  geom_tile(color = "white", linewidth = 0.5) +
  geom_text(aes(label = sprintf("%.2f", kappa)), color = "black", size = 3) +
  scale_fill_viridis_c(limits = c(-1,1), option = "plasma") +
  theme_minimal(base_size = 12) +
  ...
  ggtitle("Pixel-based")
#mcc
pixel_all3 <-  ggplot(pixel_mcc_array, aes(x = landcov, y = treecov, fill = mcc)) +
  geom_tile(color = "white", linewidth = 0.5) +
  geom_text(aes(label = sprintf("%.2f", mcc)), color = "black", size = 3) +
  scale_fill_viridis_c(limits = c(-1,1), option = "plasma") +
  theme_minimal(base_size = 12) +
...
#mature
pixel_mat <-  ggplot(pixel_mat_accuracy_array, aes(x = landcov, y = treecov, fill = accuracy)) +
  geom_tile(color = "white", linewidth = 0.5) +
  geom_text(aes(label = sprintf("%.2f", accuracy)), color = "black", size = 3) +
  scale_fill_viridis_c(limits = c(0,1), option = "plasma") +
  theme_minimal(base_size = 12) +
  ...
  ggtitle("Mature Pixel-based")
#kappa
pixel_mat2 <-  ggplot(pixel_mat_kappa_array, aes(x = landcov, y = treecov, fill = accuracy)) +
  geom_tile(color = "white", linewidth = 0.5) +
  geom_text(aes(label = sprintf("%.2f", accuracy)), color = "black", size = 3) +
  scale_fill_viridis_c(limits = c(-1,1), option = "plasma") +
  theme_minimal(base_size = 12) +
  ...
  ggtitle("Mature Pixel-based")
#mcc
pixel_mat3 <-  ggplot(pixel_mat_mcc_array, aes(x = landcov, y = treecov, fill = accuracy)) +
  geom_tile(color = "white", linewidth = 0.5) +
  geom_text(aes(label = sprintf("%.2f", accuracy)), color = "black", size = 3) +
  scale_fill_viridis_c(limits = c(-1,1), option = "plasma") +
  theme_minimal(base_size = 12) +
...
ggarrange(pixel_all, pixel_mat, parcel_all, parcel_mat, 
          common.legend = TRUE,
          labels = c("A", "B", "C", "D"))


ggsave(filename = "C:/Users/am1355/OneDrive - University of Leicester/Publications/Thesis/ch2/figures/accuracy.jpeg",
       width = 21, height = 21, units = "cm", dpi = 600)

#trying a long table with it all together
full_results <- pixel_accuracy_array
full_results$Type <- "Pixel-based"
full_results<- rbind(full_results, data.frame(pixel_mat_accuracy_array, 
                                              Type = "Mature Pixel-based"))
full_results<- rbind(full_results, data.frame(accuracy_array[-(which(accuracy_array$treecov == "NTM")),], 
                                              Type = "Parcel-based"))
full_results<- rbind(full_results, data.frame(accuracy_mat_array[-(which(accuracy_mat_array$treecov == "NTM")), c(1,2,4)], 
                                              Type = "Mature Parcel-based"))

ggplot(full_results, aes(x = landcov, y = treecov, fill = accuracy)) +
  geom_tile(color = "white", linewidth = 0.5) +
...
###OLD OMISSION AND COMISSION CODE
